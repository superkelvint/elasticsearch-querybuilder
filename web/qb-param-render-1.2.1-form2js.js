Handlebars.registerHelper('render_param', function (prefix, name, type, id) {
  ...
  if (type == 'BOOLEAN') {
    ...
    s = '<input type="button" class="form-control btn btn-info queryadd" target="'+fullname+'" targettype="filter" tgid="'+id+'" value="Add filter to '+ name+ '" id="'+fullname+'_addfilter"><ol></ol>';
  } else if (type == 'LIST_QUERY_BUILDER') {
    s = '<input type="button" class="form-control btn btn-info queryadd" target="'+fullname+'" targettype="query" tgid="'+id+'" value="Add query to '+ name+ '" id="'+fullname+'_addquery"><ol></ol>';
    ...
    s = '<input type="text" class="ref form-control" name="' + fullname + '" id="' + fullname + '">';
  }
  return s;
});